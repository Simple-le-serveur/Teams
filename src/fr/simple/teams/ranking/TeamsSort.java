	...
	public static List<String> SortTeams() {
		...
		for (int i = 0; i < teamsNoClassed.size(); i++) {
			if (i == 0) {
				...
				if (TeamData.getTeamNotation(teams.get(0)) <= TeamData.getTeamNotation(teamsNoClassed.get(i))) {
					teams.add(0, teamsNoClassed.get(i));
				} else if (TeamData.getTeamNotation(teams.get(i - 1)) >= TeamData
						.getTeamNotation(teamsNoClassed.get(i))) {
					teams.add(teamsNoClassed.get(i));
				} else {
					for (int j = 0; j < teams.size(); j++) {
						if (TeamData.getTeamNotation(teams.get(j)) >= TeamData.getTeamNotation(teamsNoClassed.get(i))
								&& TeamData.getTeamNotation(teams.get(j + 1)) <= TeamData
										.getTeamNotation(teamsNoClassed.get(i))) {
